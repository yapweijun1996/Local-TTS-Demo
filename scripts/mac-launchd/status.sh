...
echo
echo -n "Kokoro v1.1-zh sidecar (8201): "; curl -sf http://127.0.0.1:8201/health 2>/dev/null || echo "unreachable"
echo
echo -n "API (6700): "; curl -sf http://127.0.0.1:6700/health 2>/dev/null || echo "unreachable"
echo

echo ""
echo "== last 10 log lines (errors) =="
for f in "$LOG_DIR/voxcpm-sidecar.error.log" "$LOG_DIR/kokoro-zh-sidecar.error.log" "$LOG_DIR/api.error.log"; do
